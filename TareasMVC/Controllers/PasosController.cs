...
namespace TareasMVC.Controllers
{
...

    {
        ...
        public async Task<ActionResult<Paso>> Post(int tareaId, [FromBody] PasoCrearDTO pasoCrearDTO)
        {
            ...
            if (tarea == null) //Si no hay tarea
            {
                return NotFound();
            }

            if (tarea.UsuarioCreacionId != usuarioId) //Si hay tarea, pero el usuario no tiene permiso
            {
                return Forbid();
            }
        ...
        public async Task<ActionResult> Delete(Guid id)
        {
            ...
            context.Remove(paso); //remove marca la entidad para ser borrada
            await context.SaveChangesAsync(); //Se borra cuando guardamos los cambios
